...
| **LLM FP Review** | Opt-in `--llm-fp-review` (advisory) and `--llm-fp-adjudicate` (exit-affecting): ≥2-of-3 consensus flags likely false positives in the `Suspicious` queue with a hardened anti-jailbreak prompt and injection guard; adjudicate softens only the exit code (core verdict + JSON/SARIF unchanged); optional `skill-veil.fp-review.v1` JSON sidecar |
| **Executable-Script Amplifier** | Risk score x1.3 when a package references an executable script (empirically 2.12x more likely to be vulnerable), surfaced as an audited `artifact:executable_script` factor; bounded so benign low-scoring packages stay benign |
