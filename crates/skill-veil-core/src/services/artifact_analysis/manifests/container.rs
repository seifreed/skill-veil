...
pub(super) fn analyze_docker_compose(path: &Path, content: &str) -> Vec<Finding> {
    ...
    let Some(services) = yaml.get("services").and_then(Value::as_mapping) else {
        return findings;
    };
    ...
    for (service_name, service) in services {
        ...
        if mapping.contains_key(Value::String("privileged".to_string()))
            && mapping
        ...
        if let Some(env_file) = mapping.get(Value::String("env_file".to_string())) {
            findings.push(
                Finding::builder(
...
pub(super) fn docker_compose_capabilities(content: &str) -> Vec<ArtifactCapabilityFact> {
    ...
    for (_, service) in services {
        ...
        if mapping.contains_key(Value::String("ports".to_string()))
            && !capabilities.iter().any(|fact| {
        ...
        if mapping.contains_key(Value::String("env_file".to_string())) {
            capabilities.push(super::super::declared_capability(
                ArtifactCapability::SecretAccess,
            ));
        }

        if mapping.contains_key(Value::String("command".to_string()))
            || mapping.contains_key(Value::String("entrypoint".to_string()))
        {
            capabilities.push(super::super::declared_capability(
                ArtifactCapability::ProcessExecution,
            ));
...
pub(super) fn docker_compose_relations(content: &str) -> Vec<ArtifactLink> {
    ...
    for (_, service) in services {
        ...
        if mapping.contains_key(Value::String("volumes".to_string())) {
            links.push(ArtifactLink {
