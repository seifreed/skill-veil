...
pub fn derive_package_verdict(
    ...
    let declared_permissions = derive_declared_permissions(findings, primary_summary, supporting_summary);
...
fn detect_compound_verdict_reasons(
    ...
    if has_broad_permission_combo && has_high_risk_autonomy()
    {
        reasons.push(VerdictReason {
            ...
            signal_class: SignalClass::MaliciousBehavior,
            rationale: "Compound verdict: broad permissions are paired with autonomous execution semantics".to_string(),
        });
    }
        ...
        && (has_rule("MCP_REMOTE_EXEC_SURFACE") || has_rule("MCP_TOOLING_TRANSPORT_DECLARED"))
    {
        reasons.push(VerdictReason {
            ...
            category: ThreatCategory::RemoteExec,
            signal_class: SignalClass::MaliciousBehavior,
            rationale: "Compound verdict: MCP remote endpoint is paired with command or stdio execution semantics".to_string(),
        });
    }
...
fn build_blast_radius_summary(
    ...
    for finding in findings {
        ...
        if ["http://", "https://", "localhost", "127.0.0.1", "169.254.169.254", ".internal", ".local"]
            .iter()
    ...
    let level = if severe_count >= 3
        || factors.iter().any(|factor| factor == "remote execution" || factor == "data exfiltration")
    {
