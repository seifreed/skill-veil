use super::io::{load_json_reports, serialize_json_pretty, write_output_file};
...
pub(crate) fn run_baseline_create(args: BaselineCreateArgs) -> Result<()> {
    ...
    let content = serialize_json_pretty(&baseline, "Failed to serialize baseline")?;
...
pub(crate) fn run_baseline_update(args: BaselineUpdateArgs) -> Result<()> {
    ...
    let content = serialize_json_pretty(&updated, "Failed to serialize updated baseline")?;
    write_output_file(&args.output, &content)?;
    Ok(())
}
